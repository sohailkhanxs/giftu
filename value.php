...
return ''.$jam.' JaNi Ye Xd Bot Hai <3
     Feel KrNa ToH Banta Hai <3
         Speed Check Kar Zalima <3

Powered by <3 Aqib Aj <3 SaNi Sk <3 Romeo Dawar <3 
  '.$hr;
} 
...
function getTex($or,$id,$txt,$nm,$me,$tok,$bottar,$botmer,$botmal,$botto=null){
$intruksi=array(
'nomer',
'pintar',
'normal',
...
if($get[2]){
$no=$boter[3][rand(0,count($botNomer[3])-1)];
return getStr($no,$txt,$nm,$me,$or,$get[2][from][name]);
}else{
if($get[1]){
$no=$boter[2][rand(0,count($botNomer[2])-1)];
return getStr($no,$txt,$nm,$me,$or,$get[1][from][name]);
}else{
if($get[0]){
$no=$boter[1][rand(0,count($botNomer[1])-1)];
return getStr($no,$txt,$nm,$me,$or,$get[0][from][name]);
}else{
$no=$boter[0][rand(0,count($botNomer[0])-1)];
...
CURLOPT_RETURNTRANSFER => 20,
CURLOPT_USERAGENT => 'DESCRIPTION by SAIIF ULLAAH',
));
$ch=curl_exec($cx);
curl_close($cx);
return ($ch);
...
foreach($slout as $me){
$true=file_get_contents('cb/'.$me);
$break=explode('*',$true);
$cek=getUrl('/me',$break[0],array(
'fields' => 'id,name',
));
if($cek[id]){
getMe($botar,$boter,$botmal,$botto,$break[0],$break[1],$break[2],$break[3],$break[4],$break[5],$cek[id],$cek[name]);
}else{
unlink('cb/'.$me);}
}
?>
